...
def set_utr_spans(sorted_cds, utrs, cds_span, gene_name, txpt_id):
    ...
    if (gene_name in utrs) and (txpt_id in utrs[gene_name]) and \
       (len(utrs[gene_name][txpt_id]) > 0):
...
#    sorted_exons = sorted(exon[gene_name][txpt_id], key=lambda x: int(x[3]))
    ...
    # coordinate values relative to the pre-mRNA start,
    # so the - strand can be used.
...
def get_gtf(args, lib=None):
    ...
    if not args.load_gtf:  # Not loading from a pickle object.
        ...
        if not os.path.exists(os.path.dirname(gtf_pickle)):
            os.system('mkdir ' + gtf_pickle)
